...
export class ProfilesService {
  ...
  private async loadVisible(viewerId: string, targetId: string) {
    const user = await this.prisma.user.findUnique({
      where: { id: targetId },
      select: { id: true, displayName: true, avatarUrl: true, countryCode: true, kycVerified: true, status: true },
    });
  ...
  async get(viewerId: string, targetId: string) {
    ...
    const [followerCount, followingCount, follow, live] = await Promise.all([
