...
import { computeMultipliers } from './lucky-number/lucky-number-math';
...
const SHAPES = {
  dice: ['payoutMultiplier', 'diceCount', 'diceSides', 'numberPayouts', 'rtp', 'basePrize', 'stakeWeightExponent', ...SHARED],
...
export function validateGameRules(existing: any, incoming: any): GameRules {
  ...
  if (shape === 'dice') {
    ...
    // RTP mode ("Lucky Number" per-number stakes): admin sets a target
    // return-to-player and a base prize; every number's multiplier is
    // *derived*, not admin-entered, by flooring rtp/p(n) the same way
    // lucky-number-math.ts does for entry placement and settlement. This
    // is deliberately the single source of truth for that formula — if it
    // were duplicated here, the two could drift and an entry could be
    // priced against different odds than it settles against.
    const usesRtpMode = incoming.rtp !== undefined;
    if (usesRtpMode) {
      if (incoming.numberPayouts !== undefined) {
        errors.push('numberPayouts is derived automatically from rtp — do not set both');
      }
      if (!isNum(incoming.rtp) || incoming.rtp <= 0 || incoming.rtp > 1) {
        errors.push('rtp must be a number greater than 0 and at most 1 (e.g. 0.95 for 95%)');
      }
      if (!isInt(incoming.basePrize, 1, 1_000_000_000)) {
        errors.push('basePrize must be a whole number of coins, at least 1');
      }
      if (incoming.stakeWeightExponent !== undefined && (!isNum(incoming.stakeWeightExponent) || incoming.stakeWeightExponent < 0.1 || incoming.stakeWeightExponent > 5)) {
        errors.push('stakeWeightExponent must be a number from 0.1 to 5');
      }
      // payoutMultiplier becomes a display-only fallback in this mode (every
      // real number has its own derived multiplier), so it's optional here —
      // unlike the fixed-multiplier mode below, where it's the only payout
      // rule that exists and so must be present.
      if (incoming.payoutMultiplier !== undefined && (!isNum(incoming.payoutMultiplier) || incoming.payoutMultiplier < 1.01 || incoming.payoutMultiplier > 1000)) {
        errors.push('payoutMultiplier must be a number from 1.01 to 1000');
      }
      if (!errors.length) {
        out.rtp = incoming.rtp;
        out.basePrize = incoming.basePrize;
        if (incoming.stakeWeightExponent !== undefined) out.stakeWeightExponent = incoming.stakeWeightExponent;
        if (incoming.payoutMultiplier !== undefined) out.payoutMultiplier = incoming.payoutMultiplier;
        out.diceCount = incoming.diceCount;
        out.diceSides = incoming.diceSides;
        const multipliers = computeMultipliers(incoming.rtp);
        out.numberPayouts = Object.fromEntries([...multipliers].map(([n, m]) => [String(n), m])) as any;
      }
    } else {
