 ...
 * players never see a hit pay less than the advertised prize.
 */
...
export function computeSuggestedStakes(multipliers: Map<number, number>, basePrize: number): Map<number, number> {
  const stakes = new Map<number, number>();
  for (const [n, m] of multipliers) stakes.set(n, suggestedStake(m, basePrize));
  return stakes;
}
...
export function planStakes(
  ...
  return { stakes, total: suggestedTotal(selected, stakes) };
}
